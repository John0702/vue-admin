  ...
  <div class="content">
    <el-card shadow="never">
      ...
      <el-table ref="table" :data="nowPageData" border stripe>
        ...
        <el-table-column prop="titleUrl" label="文章封面" width="100">
        ...
        <el-table-column label="操作" width="360">
          <template slot-scope="scope">
            <el-button
              type="success"
              size="small"
              @click="changeView('/article/update', { id: scope.row.id })"
              >编辑</el-button
            >
            <el-button
              type="danger"
...
export default {
  ...
  methods: {
    ...
    //搜索
    handleSearch() {
      const { name='', code='', author='' } = this.searchForm;
      // 过滤数据
    ...
    deleteArticle(id) {
      this.$confirm("确认要删除该文章吗, 是否继续?", "提示", {
        ...
        .then(() => {
          // 删除逻辑
          let newData = JSON.parse(localStorage.getItem('articleData')).filter(item=>item.id!=id);
          localStorage.setItem('articleData',JSON.stringify(newData));
          this.articleData = newData;
          this.total = this.articleData.length;
          this.nowPageData = this.articleData.slice((this.searchForm.current-1)*this.searchForm.size, this.searchForm.current*this.searchForm.size);
